 ...
 */
static int send_to_socket(socket_t sockfd, const char *message)
{
	if (SOCKET_IS_INVALID(sockfd) || !message)
		return -1;

	size_t len = strlen(message);
	socket_io_result_t bytes_sent = platform_socket_send(sockfd, message, len);

	if (bytes_sent < 0)
	{
		LOG_ERROR("Failed to send to socket %lld: %s",
				  SOCKET_ID(sockfd), platform_socket_error_message());
		return -1;
	}

	LOG_DEBUG("Sent %lld bytes to socket %lld", (long long)bytes_sent, SOCKET_ID(sockfd));
	return 0;
}
 ...
 */
static int handle_login(socket_t client_fd, Message *msg)
{
	...
	{
		// 认证成功
		LOG_INFO("User logged in successfully: %s (fd=%lld)", username, SOCKET_ID(client_fd));
	...
	{
		// 认证失败
		LOG_WARN("Login failed for user: %s (fd=%lld)", username, SOCKET_ID(client_fd));
 ...
 * @return int 成功返回0，失败返回-1
 */
static int handle_logout(socket_t client_fd, Message *msg)
{
	...
	LOG_INFO("User logged out: %s (fd=%lld)", username, SOCKET_ID(client_fd));
	return 0;
}
 ...
 * @return int 成功返回0，失败返回错误码
 */
static int handle_send_message(socket_t client_fd, Message *msg)
{
	...
	{
		LOG_WARN("Unauthorized message attempt from fd=%lld", SOCKET_ID(client_fd));
	...
	if (!sender)
	{
		LOG_ERROR("Failed to get sender username for fd=%lld", SOCKET_ID(client_fd));
		return -1;
	}
 ...
 * @return int 成功返回0，失败返回错误码
 */
static int handle_broadcast(socket_t client_fd, Message *msg)
{
	...
	if (!session_manager_is_authenticated(client_fd))
	{
		LOG_WARN("Unauthorized broadcast attempt from fd=%lld", SOCKET_ID(client_fd));
		char *error_msg = build_error_msg(ERROR_AUTH_FAILED, "Please login first");
		if (error_msg)
		{
			send_to_socket(client_fd, error_msg);
 ...
 * @return int 暂返回-1
 */
static int handle_history_request(socket_t client_fd, Message *msg)
{
	...
	if (!session_manager_is_authenticated(client_fd))
	{
		LOG_WARN("Unauthorized history request from fd=%lld", SOCKET_ID(client_fd));
		char *error_msg = build_error_msg(ERROR_AUTH_FAILED, "Please login first");
		if (error_msg)
		{
			send_to_socket(client_fd, error_msg);
	...
	char *content_copy = platform_strdup(msg->content);
	if (!content_copy)
	{
 ...
 * @return int 成功返回0，失败返回-1
 */
static int handle_status_request(socket_t client_fd, Message *msg)
{
 ...
 * @return int 暂返回-1
 */
static int handle_group_message(socket_t client_fd, Message *msg)
{
	...
	if (!session_manager_is_authenticated(client_fd))
	{
		LOG_WARN("Unauthorized group message attempt from fd=%lld", SOCKET_ID(client_fd));
		char *error_msg = build_error_msg(ERROR_AUTH_FAILED, "Please login first");
		if (error_msg)
		{
			send_to_socket(client_fd, error_msg);
 ...
 * @return int 成功返回0，失败返回错误码
 */
int handle_command(socket_t client_fd, Message *msg)
{
	if (!msg)
	{
	...
	LOG_DEBUG("Handling command: fd=%lld, type=%s", SOCKET_ID(client_fd), msg->type);
 ...
 * @return int 成功返回0，失败返回错误码
 */
int handle_raw_message(socket_t client_fd, const char *raw_message)
{
	if (!raw_message || SOCKET_IS_INVALID(client_fd))
	{
